...
bar_perfor <- billboard %>%
  ...
  select(year,performer,week_position) %>%
  na.omit() %>%
  filter(year >= 2010 & year <= 2020, performer %in% top_5_perfor,week_position <= 20) %>%
  group_by(performer,year) %>%
...
ggplot(bar_missing, aes(x = year, y = appear, fill = year)) +
  ...
  labs(x = "Performer", y = "Number of Times the performer appear as Top 20 on Billboard", fill = "Year") +
  ggtitle("How did the Top 5 performer appearance frequency change over the years (2010-2020)") +
